...
- **Local path**: **`/mnt/user/AI/Projects/league-manager` on the Unraid server (host `unraid`, 192.168.86.230) is the PRIMARY dev checkout since 2026-08-31.** The Mac copy `~/Projects/league-manager` is kept in sync but is no longer where work happens; `~/Developer/league-manager` (stale v1.24.0) and `~/Nextcloud/AI/league-manager` (empty) are dead decoys — check `git log -1` before touching any handed-over folder. On the Unraid share git needs `safe.directory` (set in `/boot/config/go`) and `core.fileMode false` in the repo, or `git status` reports ~860 phantom modified files that are only permission bits.
- **GitHub**: https://github.com/halvar20000/simracing-hub-league-manager — branch `main`
- **Deploy flow (LIVE = Hetzner/Coolify)**: the live site `league.simracing-hub.com` is served by **Coolify on a Hetzner CX23** (IP `5.75.174.170`); DB = **Coolify-managed Postgres** on the same box. DNS is a Cloudflare **`league` A-record -> 5.75.174.170, DNS-only (grey cloud, NOT proxied)** — leave the apex/`www` GitHub-Pages records alone. **Deploys ARE automatic: a push to `main` triggers a Coolify build and redeploy on its own** (re-verified 2026-08-31) — no manual redeploy and no Coolify API token needed. Claude sessions on both the Unraid box and the Mac can `git push` directly; the old "the sandboxed Claude environment cannot push — write a shell script to `outputs/` for the user to run" rule is OBSOLETE. Both machines also have root SSH to the box: `ssh -i ~/.ssh/hetzner_cls root@5.75.174.170`. Coolify dashboard: `http://5.75.174.170:8000` -> project "My first project" -> app `league-manager`.
...
- **Standard footer for change scripts**: `npx tsc --noEmit -p tsconfig.json` -> `git add <paths>` -> `git commit -m "..."` -> `git push`. **The push IS the deploy** — Coolify rebuilds by itself; confirm the new build went live with `curl https://league.simracing-hub.com/api/build-id`.
...
- **`marginLap`** takes one lap off the FUEL-LIMITED stint length (not litres off the tank). **No control any more since 2.21.0** — the name "Margin Lap" went to the +1-lap finish (`roundRaceEnd`) and two controls cannot share it. The engine still honours a stored `true` so an archived payload computes as signed off; no live plan had it on. `fuelReserve` is the fixed-litre equivalent and is still on screen.

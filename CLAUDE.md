...
- **Rule**: per registration, walk rounds with `status = COMPLETED` that the driver **started** (a `RaceResult` with `finishStatus` in `CLASSIFIED | DNF | DSQ` — DSQ counts because forgiveness tracks penalty POINTS, not result status; only `DNS`/no-result are excluded). If a new `Penalty` (type=POINTS_DEDUCTION, pointsValue>0) was issued in that round → clean counter resets to 0. Otherwise if pool > 0, counter++; at counter = 2, forgive 1 from the oldest non-fully-forgiven penalty (FIFO) and reset counter. Stops when pool reaches 0.
- **Auto-triggers**:

#!/usr/bin/env bash
# Consolidate PCCD 5th season's 16 rounds into 8 (each old pair becomes a
# single round with raceLengthMinutes=50, ready for 2 races per round
# imported from iRLM).
set -euo pipefail
cd "$HOME/Nextcloud/AI/league-manager"

mkdir -p scripts
cat > scripts/consolidate-pccd-s5.ts <<'EOF'
...
EOF
npx tsx scripts/consolidate-pccd-s5.ts

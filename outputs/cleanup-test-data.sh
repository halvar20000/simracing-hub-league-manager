#!/usr/bin/env bash
# Wipe the test data from the current GT3 WCT test season so we can
# start fresh. Preserves:
#   - The Season + Rounds
# Deletes:
#   - All Registrations in the season
#   - All RaceResults for those rounds
#   - All Teams in the season
#   - All CarClasses in the season
#   - All FPRAwards for those rounds
#   - All CsvImport audit records for those rounds
#   - Test Users (any user with email ending in @test.simracing-hub.com)
#
# Usage:
...
cat > scripts/clean-test-data.ts <<'EOF'
...
echo "Wrote scripts/clean-test-data.ts"
echo ""
echo "Now run:"
echo "  npx tsx scripts/clean-test-data.ts"

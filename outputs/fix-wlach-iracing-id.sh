#!/usr/bin/env bash
# Fix duplicate Bernhard Wlach: SFL S7 registration was created against a
# bogus User (iRacing 710028). The real iRacing ID is 841362, which already
# exists from WCT12 setup. Re-point the SFL registration to the correct User
# and delete the duplicate.
set -euo pipefail
cd "$HOME/Nextcloud/AI/league-manager"

mkdir -p scripts
cat > scripts/fix-wlach-id.ts <<'EOF'
...
EOF
npx tsx scripts/fix-wlach-id.ts

echo ""
echo "Done. Now re-pull each completed SFL round in admin so Bernhard's results"
echo "get imported (the iRLM importer will now find his Registration by"
echo "iracingMemberId=841362)."

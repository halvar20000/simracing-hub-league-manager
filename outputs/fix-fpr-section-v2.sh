#!/usr/bin/env bash
# v2: write the patch as a standalone Node script so no escaping issues.
set -euo pipefail
cd "$HOME/Nextcloud/AI/league-manager"

mkdir -p scripts

cat > scripts/patch-fpr.mjs <<'PATCH_EOF'
...
echo "Running scripts/patch-fpr.mjs ..."
node scripts/patch-fpr.mjs

echo ""
echo "Confirm no a.user / a.fprPoints / a.registration left:"
grep -n 'a\.user\|a\.fprPoints\b\|a\.registration' \
  'src/app/leagues/[slug]/seasons/[seasonId]/rounds/[roundId]/page.tsx' \
  || echo "  (clean)"

echo ""
echo "Confirm include shape:"
grep -n 'fprAwards' \
  'src/app/leagues/[slug]/seasons/[seasonId]/rounds/[roundId]/page.tsx'
...
git add -A
git commit -m "Fix FPR section: team award, correct include + fields (v2)"
git push

#!/usr/bin/env bash
# Rewrite irlm-import.ts cleanly with full multi-race support.
set -euo pipefail
...
echo ""
echo "Rewrote src/lib/actions/irlm-import.ts."

echo ""
echo "Sanity check:"
grep -n 'raceNumber' src/lib/actions/irlm-import.ts | head -10

echo ""
echo "=== Commit and push ==="
git add -A
git commit -m "iRLM importer: clean multi-race rewrite (parse Race 1/Race 2 sessions)"
git push

echo ""
echo "Done. Wait ~60s for Vercel."
echo ""
echo "Once green, re-pull each completed SFL round in admin to populate"
echo "Race 1 + Race 2 separately. GT3/GT4 imports unchanged (default raceNumber=1)."

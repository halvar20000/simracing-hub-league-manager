#!/usr/bin/env bash
# Create CAS PCCD Season 04:
#   - existing CAS PCCD league + scoring system
#   - 8 rounds, Thursdays (assumed) at 19:00 CET/CEST, 50 min total race
#     (2 x 25 min sprints)
#   - racesPerRound=2 on the scoring system (and copy SFL's points tables
#     as a starting point — you can tune them at /admin/scoring-systems)
#   - 21 drivers from the Google Sheet (PCCD- Anmeldung Formular Season 04)
#   - Status mapping:
#       Ligaeinladung "Einladung offen" -> PENDING
#       Otherwise (Member Season 3 / Angenommen) -> APPROVED
set -euo pipefail
cd "$HOME/Nextcloud/AI/league-manager"

mkdir -p scripts

cat > scripts/create-pccd-s4.ts <<'EOF'
...
echo "Running scripts/create-pccd-s4.ts ..."
npx tsx scripts/create-pccd-s4.ts

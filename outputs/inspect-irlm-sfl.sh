#!/usr/bin/env bash
# Phase 2: walk an SFL iRLM event and dump every session's identity + counts
# so we know how Race 1 / Race 2 are exposed.
set -euo pipefail
cd "$HOME/Nextcloud/AI/league-manager"

# Load .env so IRLM_USERNAME / IRLM_PASSWORD reach the script
if [ -f .env ]; then
  set -a
...
mkdir -p scripts
cat > scripts/inspect-irlm-sfl.ts <<'EOF'
...
if [ -z "${IRLM_USERNAME:-}" ] || [ -z "${IRLM_PASSWORD:-}" ]; then
  echo "Run aborted: IRLM_USERNAME / IRLM_PASSWORD missing in .env."
  exit 1
fi

npx tsx scripts/inspect-irlm-sfl.ts

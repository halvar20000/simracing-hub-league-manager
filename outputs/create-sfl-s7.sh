#!/usr/bin/env bash
# Create the CAS SFL Cup 7th Season:
#  - Season under existing CAS SFL Cup league + CAS SFL Cup scoring system
#  - 8 rounds, Wednesdays 19:00 CET/CEST, raceLength 40 min (2 x 20 min sprints)
#  - Status COMPLETED for past rounds, UPCOMING for future (relative to today)
#  - 26 drivers from the Anmeldung CSFL Season 7 sheet
#  - Status mapping:
#       "Startberechtigt"          -> APPROVED
#       "Aus der Liga abgemeldet"  -> WITHDRAWN
#       (empty)                    -> PENDING
set -euo pipefail
...
cat > scripts/create-sfl-s7.ts <<'EOF'
...
echo "Running scripts/create-sfl-s7.ts ..."
npx tsx scripts/create-sfl-s7.ts

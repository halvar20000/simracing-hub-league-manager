#!/usr/bin/env bash
# Re-rank class standings within their own class.
# Pro and AM standings now compute race points by re-ranking each round's
# results within their class only — so a Pro who finished 4th overall but
# was the 2nd Pro on track receives the points for 2nd place in Pro standings.

set -euo pipefail
cd "$HOME/Nextcloud/AI/league-manager"

# ------------------------------------------------------------
# 1. Rewrite standings.ts with class-position-based scoring
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# 2. Standings page — use classRawPoints in class views
# ------------------------------------------------------------
echo ">>> Updating standings page to use classRawPoints in class views..."
...
// In the DriversTable list view: choose raw based on kind for the Raw column
s = s.replace(
...
// Replace the cell that previously used r.rawPoints for the Raw column
s = s.replace(
  /<ValueCell value=\{r\.rawPoints\} delta=\{rawDelta\} \/>/,
  '<ValueCell value={rawForView} delta={rawDelta} />'
);

// In RaceByRaceTable's R sub-column, use classRawPoints in class kind
s = s.replace(
  /<td className=\"px-1\.5 py-1\.5 text-right tabular-nums text-zinc-300\">\s*\{rp\.hasResult && rp\.rawPoints !== 0 \? rp\.rawPoints : dash\}\s*<\/td>/,
...
echo ""
echo "Done. Refresh the standings page."
echo "Pro + AM views now use re-ranked class positions for their race points."
echo "Combined view stays as overall-position scoring."

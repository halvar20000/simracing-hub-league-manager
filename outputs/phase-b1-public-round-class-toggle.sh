#!/usr/bin/env bash
# Phase B1: Combined / By class toggle on the PUBLIC round page.
# Single-class seasons keep working as-is (toggle hidden).
# In by-class mode: Pro first, AM next, then other classes by displayOrder.
# Class position is renumbered 1..N for CLASSIFIED rows in each class.
...
# Easiest reliable patch: rewrite the file. We just verified its current
# structure. Below is the new version with the same imports, query, winner
# block and FPR section, plus the toggle and grouping logic.
cat > "$PAGE" <<'EOF'
...
echo ""
echo "=== Sanity check ==="
echo "First imports of public round page:"
head -7 "$PAGE"
echo ""
echo "Confirm by-class branch + ResultsTable component exist:"
grep -n 'cls === \"byclass\" && isMulticlass\|function ResultsTable\|renumberWithinGroup' "$PAGE" | head -10

echo ""
echo "=== Commit and push ==="
git add -A
git commit -m "Public round page: Combined / By class toggle"
git push
...
echo ""
echo "Test:"
echo "  1. Reload the public round page — default view (Combined) should look identical to before."
echo "  2. Click 'By class' — you should see a 'PRO' table then an 'AM' table, each numbered 1..N."
echo "  3. On a single-class round, the toggle should not appear at all."
echo ""
echo "Once you confirm, send me a dump of the ADMIN round page registration loop and I'll do Phase B2."

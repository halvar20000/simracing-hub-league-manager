#!/usr/bin/env bash
# Step 1: Create PRO and AM CarClass rows for the WCT12 season.
# Step 2: Assign each Registration to its class based on the WCT12
#         Google Sheet (iRacing ID -> PRO/AM mapping).
# Step 3: Recompute scoring on every round that has results.
set -euo pipefail
...
cat > scripts/setup-wct12-classes.ts <<'EOF'
...
echo "Running scripts/setup-wct12-classes.ts ..."
SEASON_ID="$SEASON_ID" npx tsx scripts/setup-wct12-classes.ts

echo ""
echo "Done."
echo "Reload the public round page — you'll now see Pro and Am rows correctly grouped"
echo "in 'By class' view. Then I'll do the 4-button toggle + Team view as Phase B2."

#!/usr/bin/env bash
# Find Justin Jensen in iRLM (GT3 WCT league members) and add him to
# CAS GT3 WCT 12 season as APPROVED + excludedAt set.
# Then prompt the user to re-pull rounds so his historical results land.
set -euo pipefail
cd "$HOME/Nextcloud/AI/league-manager"

# Load .env so iRLM creds reach the script
if [ -f .env ]; then
  set -a
...
cat > scripts/add-jensen-to-wct.ts <<'EOF'
...
npx tsx scripts/add-jensen-to-wct.ts

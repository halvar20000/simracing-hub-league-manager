#!/usr/bin/env bash
# Create the GT4 Masters 3rd Season:
#  - Season under existing CAS TSS GT4 league, using existing CAS GT4 Masters scoring
#  - 8 rounds (Mondays 18:45 CET/CEST), all marked COMPLETED (last race was 27.04.2026)
#  - 33 drivers from the registration sheet — upserted as Users (4 already exist)
#  - Teams created per-season as needed
#  - Status mapping:
#       "Teilnehmer Season 2", "Einladung angenommen" -> APPROVED
#       "Abgemeldet"                                  -> WITHDRAWN
#       "Aus Liga ausgeschlossen"                     -> REJECTED
set -euo pipefail
cd "$HOME/Nextcloud/AI/league-manager"

mkdir -p scripts

cat > scripts/create-gt4-s3.ts <<'EOF'
...
echo "Running scripts/create-gt4-s3.ts ..."
npx tsx scripts/create-gt4-s3.ts

#!/usr/bin/env bash
# Phase B2: 4-button toggle (Combined / Pro / Am / Team) on the public round
# page, with an expandable team view (top-N driver points = team total).
#
# - Combined  : single overall table (current behaviour kept)
# - Pro / Am  : single table filtered to that class, positions renumbered 1..N
# - Team      : list of team rows, click to expand and see drivers
set -euo pipefail
...
echo ""
echo "Toggle pills:"
grep -n '?cls=pro\|?cls=am\|?cls=team\|cls === "team"\|cls === "pro"\|cls === "am"' "$PAGE" | head -10
echo ""
echo "Team view component:"
grep -n 'function TeamView\|TEAM_BEST_N\|topNTotal' "$PAGE" | head -8

echo ""
echo "=== Commit and push ==="
git add -A
git commit -m "Public round page: 4-button toggle (Combined/Pro/Am/Team) + expandable team view"
git push
...
echo ""
echo "Then test:"
echo "  Combined  -> overall table (will still show duplicate positions; we'll fix that separately)"
echo "  Pro       -> only Pro drivers, positions renumbered 1..N"
echo "  Am        -> only Am drivers, positions renumbered 1..N"
echo "  Team      -> rows per team sorted by Top-2 points; click a team to expand drivers"

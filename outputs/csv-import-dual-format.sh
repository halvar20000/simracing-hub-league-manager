#!/usr/bin/env bash
# Extend the CSV import to handle both iRacing hosted-session exports AND
# iRLeagueManager combined-race-results exports.
#
# Auto-detects the format from headers + delimiter:
#   - "Fin Pos" + "Cust ID"           → iRacing (comma, match by Cust ID)
#   - "Position;Name;...;Race Points" → iRLeagueManager (semicolon, match by Name)
...
echo "Done. CSV parser now auto-detects iRacing vs iRLeagueManager format."
echo "Restart the dev server (Ctrl-C / npm run dev) and try uploading."

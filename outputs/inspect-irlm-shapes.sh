#!/usr/bin/env bash
# Dump one iRLM result row and one Member entity so we can see which field
# carries the real iRacing CustID, and confirm the right Members endpoint URL.

set -euo pipefail
cd "$HOME/Nextcloud/AI/league-manager"

LEAGUE_NAME="GT3WCTSeason2"      # adjust if your iRLM league has a different short name
EVENT_ID="2645"                  # the event you just pulled

mkdir -p scripts
cat > scripts/inspect-irlm-shapes.ts <<'EOF'
...
echo "Wrote scripts/inspect-irlm-shapes.ts"
echo "Running with LEAGUE_NAME=$LEAGUE_NAME EVENT_ID=$EVENT_ID ..."

#!/usr/bin/env bash
# Add per-row deltas (position / Inc / Pen / Raw / Total) to the standings list view.
# Deltas compare current standings vs standings with the latest round's results excluded.
...
# ------------------------------------------------------------
# 1. standings library — accept an excludeRoundIds option
# ------------------------------------------------------------
echo ">>> Patching standings library to support excludeRoundIds..."
...
// Add the optional parameter to the signature
s = s.replace(
  /export async function computeDriverStandings\(\s*prisma: PrismaClient,\s*seasonId: string\s*\): Promise<DriverStanding\[\]>/,
...
// Replace the registrations + rounds query to filter raceResults / penalties
s = s.replace(
  /const \[registrations, rounds\] = await Promise\.all\(\[\s*prisma\.registration\.findMany\(\{\s*where: \{ seasonId, status: \"APPROVED\" \},\s*include: \{\s*user: true,\s*team: true,\s*carClass: true,\s*raceResults: \{ include: \{ round: true \} \},\s*penalties: \{ where: \{ type: \"POINTS_DEDUCTION\" \} \},\s*\},\s*\}\),\s*prisma\.round\.findMany\(\{\s*where: \{ seasonId \},\s*orderBy: \{ roundNumber: \"asc\" \},\s*select: \{ id: true, roundNumber: true, name: true \},\s*\}\),\s*\]\);/,
...
# ------------------------------------------------------------
# 2. Standings page — fetch previous standings, render deltas
# ------------------------------------------------------------
echo ">>> Rewriting standings page with delta indicators..."
...
echo ""
echo "Done. Refresh the standings page — you'll see colored deltas next to Pos / Inc / Pen / Raw / Total comparing against the standings before the most recent round."

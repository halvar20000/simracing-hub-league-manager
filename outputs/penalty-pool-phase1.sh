...
# ============================================================================
# 1. Schema: add autoForgivenPoints to Penalty
# ============================================================================
echo "=== 1. Patch prisma/schema.prisma ==="
node -e "
const fs = require('fs');
const FILE = 'prisma/schema.prisma';
let s = fs.readFileSync(FILE, 'utf8');
if (s.includes('autoForgivenPoints')) {
  console.log('  Already has autoForgivenPoints.');
  process.exit(0);
}
const before = s;
s = s.replace(
...
if (s === before) {
  ...
  s = s.replace(
    /(forgivenPoints\s+Int\s+@default\(0\))/,
    '\$1\n  autoForgivenPoints Int @default(0)'
  );
}
if (s === before) {
  console.error('  Anchor (forgivenPoints) not found in Penalty model.');
  process.exit(1);
}
fs.writeFileSync(FILE, s);
console.log('  Added autoForgivenPoints Int @default(0)');
"

echo ""
echo "-- Verify --"
grep -nE 'forgivenPoints|autoForgivenPoints' prisma/schema.prisma | head -6
...
npx --yes prisma migrate dev --name penalty_pool_auto_forgiveness || {
  echo "!!! prisma migrate failed. NOT pushing."
  exit 1
}

# ============================================================================
# 3. Engine: src/lib/penalty-pool.ts
# ============================================================================
echo ""
echo "=== 3. Write src/lib/penalty-pool.ts ==="
mkdir -p src/lib
  ...
  for (const reg of registrations) {
    const myPenalties = allPenalties
      ...
      .map((p) => ({
        ...
        autoForgiven: 0, // we just reset; tracked locally for this run
      }));
...
# ============================================================================
# 4. Server action wrapper for "Recompute" button
# ============================================================================
echo ""
echo "=== 4. Write src/lib/actions/penalty-pool.ts ==="
...
git add -A
git status --short
git commit -m "Penalty pool (Phase 1, GT3 WCT): add Penalty.autoForgivenPoints + recomputePenaltyPoolForSeason engine + server action. No UI/auto-triggers yet."
git push

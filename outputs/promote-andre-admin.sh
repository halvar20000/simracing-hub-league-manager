...
# Pass --confirm to actually write; otherwise dry-run.
MODE="${1:-}"

mkdir -p scripts
cat > scripts/lm_promote_andre.ts <<'TS'
...
if [ "$MODE" = "--confirm" ]; then
  npx --yes tsx scripts/lm_promote_andre.ts --confirm
else
  npx --yes tsx scripts/lm_promote_andre.ts
fi
